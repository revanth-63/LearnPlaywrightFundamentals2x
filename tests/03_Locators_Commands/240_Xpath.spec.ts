...
test("TC#1 - Verify that the app.vwo login is not working and gives you an error with lazy, strict, and auto wait concepts", async ({ page }) => {
    ...
    let userNameFieldXpath = page.locator('xpath=//a[@id="btn-make-appointment"]');
    let userNameFieldCSS = page.locator('css=//a[@id="btn-make-appointment"]');
    ...
    await expect(error_message).toContainText("Your email, password, IP address or ");
});